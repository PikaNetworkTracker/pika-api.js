...
module.exports = class PikaNetwork {
  ...
  async getLeaderboard(options) {
    ...
    if (nullOrUndefined(options.offset)) options.offset = 0;
   ...
   * @returns {import("../types/index.js").FactionsTop}
   */
   ...
   * @returns {import("../types/index.js").Count}
   */
  async getCount() {
    const [res, mcstatus, discord] = await Promise.all([
      queue("https://api.craftigames.net/count/play.pika-network.net"),
      queue("https://api.mcstatus.io/v2/status/java/play.pika-network.net"),
      queue("https://discord.com/api/guilds/234731357903781888/widget.json")
    ]);
    const data = {
        ip: res.ip,
        minecraftCount: mcstatus.players.online,
        minecraftMax: mcstatus.players.max,
        discordCount: discord.presence_count,
