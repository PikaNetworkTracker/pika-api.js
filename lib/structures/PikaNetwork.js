...
module.exports = class PikaNetwork {
  ...
  async getProfile(username) {
    ...
    if (!parsed.success) throw new PikaNetworkError(fromZodError(parsed.error));
    const { data } = parsed;
    const res = await queue(`${this.BASE_URL}/profile/${data}`);
  ...
  async getRecap(id) {
    ...
    if (!parsed.success) throw new PikaNetworkError(fromZodError(parsed.error));
    const res = await queue(`${this.BASE_URL}/recaps/${parsed.data}`);
    return res.body.json().catch((e) => {
      if (e.message === "Unexpected end of JSON input")
  ...
  async getFactionsTop() {
    const res = await queue(`${this.BASE_URL}/factionstop?type=opfactions`);
    return res.body.json();
  }
  ...
  async getCount() {
    ...
    /**
     * @type {Omit<import("../types/index.js").Count, "updatedAtDate">}
     */
    const body = await res.body.json();
    return {
      ...body,
      updatedAtDate: new Date(body.updated_at),
    };
  }
};
